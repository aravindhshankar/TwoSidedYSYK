...
for i, lamb in enumerate(lamblist):
	...
	ax2.semilogy(xaxis, plottable[startT:stopT:skip],'p',label = 'Exact numerics ',c='C0')
	# ax2.plot(xaxis, plottable[startT:stopT],'p',label = 'numerics GDtau',markersize=2,c='C2')
	ax2.axvline(l0, ls=':',c='C4')
	ax2.axvline(l1, ls=':',c='C4')
	...
	gamma = slope / beta
	A = np.exp(logA)
	ax2.semilogy(xaxis, A*np.exp(-slope * xaxis),ls='--', label = f' fit with slope {gamma:.4}', c='C4')
	c = gamma/delta
	...
	B  = np.exp(logB) 
	zeta = - sec_slope/beta
	ax2.semilogy(xaxis, (B)*np.exp(-(zeta)* beta * xaxis),ls='--', c='C2', label = f' fit with slope {zeta:.4}')
	print(f'A = {A:.4}, B = {B:.4}') 
	...
	ax2.semilogy(xaxis, remnant2, c='C1', ls='-', label = f' Second Remnant')
	...
	textstr += f'c = $\\gamma/\\Delta = $ {c:.4} \n'
	textstr += f'next slope at $c(1+\\Delta ) = $ {c*(1+delta):.4} \n'
	textstr += f'Fit second slope = {zeta:.4}'
	# textstr = f'expected spacing {c} \n' + 
	ax2.text(0.28, 0.9, textstr, transform=ax2.transAxes, fontsize=10, verticalalignment='top')
...
# lgd = fig2.legend(handles, labels, ncol=len(labels)//2, loc="lower center", bbox_to_anchor=(1,-0.35),frameon=True,fancybox=True,borderaxespad=0, bbox_transform=ax2.transAxes)
lgd = fig2.legend(handles, labels, ncol=2, loc="lower center", bbox_to_anchor=(0.5,-0.35),frameon=True,fancybox=True,borderaxespad=0, bbox_transform=ax2.transAxes)
