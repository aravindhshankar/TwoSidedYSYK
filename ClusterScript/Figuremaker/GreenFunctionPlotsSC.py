...
titlestring = r'$\lambda = 0.05 \,\,\,\, \beta_{WH} \approx 64 \,\,\,\, \beta_{SC} \approx 32 $'
figLL.suptitle(r'$\lambda = 0.05 \,\,\,\, \beta_{WH} \approx 64 \,\,\,\, \beta_{SC} \approx 32 $')
...
# figSL,axSL = plt.subplots(1,4)
figSL,axSL = plt.subplots(1,3)
...
for i, beta in enumerate(betalist): 
	...
	ax1.set_ylabel(r'$-g^2\,\Im{G_{d}(\omega_n)}$')
	...
	ax3.set_ylabel(r'$-g^2\,|F_{d}(\omega_n)|$')
	...
	axSL[0].semilogy(tau[startT:stopT]/beta, np.abs(np.real(GDtau[startT:stopT])),'p',c=col,label = lab)
	#axSL[0].semilogy(tau[startT:stopT], conf_fit_GD[startT:stopT],'k--',label = 'ES power law')
	#axSL[0].semilogy(tau[startT:], -np.imag(Gconf[startT:]),'m.',label = 'ES solution')
	#axSL[0].semilogy(tau[startT:], alt_conf_fit_G[startT:],'g--', label = 'alt power law')
	#axSL[0].set_xlim(tau[startT]/2,tau[startT+15])
	# axSL[0].semilogy(tau[startT:stopT]/beta, np.exp(mT*tau[startT:stopT] + cT), c=col,label=f'Fit with slope {mT:.03f}')
	#axSL[0].set_ylim(1e-1,1e1)
	axSL[0].set_xlabel(r'$\tau/\beta$')
	axSL[0].set_ylabel(r'$-\Re G_{d}(\tau)$')
	#axSL[0].set_aspect('equal', adjustable='box')
	#axSL[0].axis('square')
	axSL[0].legend()
	axSL[0].set_yscale('log')


	axSL[1].semilogy(tau[startT:stopT]/beta, np.abs(np.real(DDtau[startT:stopT])),'p',c=col,label=lab)
	#axSL[1].semilogy(tau[startB:stopB], conf_fit_DD,'k--',label = 'ES power law')
	#axSL[1].semilogy(tau[startB:], np.real(Dconf[startB:]),'m.',label = 'ES solution')
	#axSL[1].semilogy(tau[startB:], alt_conf_fit_D,'g--', label = 'alt power law')
	#axSL[1].set_xlim(tau[startB]/2,tau[startB+15])
	#axSL[1].set_ylim(5e-1,100)
	axSL[1].set_xlabel(r'$\tau/\beta$')
	axSL[1].set_ylabel(r'$\Re{D_{d}(\tau)}$',labelpad = None)
	#axSL[1].set_aspect('equal', adjustable='box')
	axSL[1].legend()


	axSL[2].semilogy(tau[startT:stopT]/beta, np.abs(np.abs(FDtau[startT:stopT])),'p',c=col,label = lab)
	#axSL[2].semilogy(tau[startT:stopT], conf_fit_GD[startT:stopT],'k--',label = 'ES power law')
	#axSL[2].semilogy(tau[startT:], -np.imag(Gconf[startT:]),'m.',label = 'ES solution')
	#axSL[2].semilogy(tau[startT:], alt_conf_fit_G[startT:],'g--', label = 'alt power law')
	#axSL[2].set_xlim(tau[startT]/2,tau[startT+15])
	# axSL[2].semilogy(tau[startT:stopT]/beta, np.exp(mT*tau[startT:stopT] + cT), c=col,label=f'Fit with slope {mT:.03f}')
	# axSL[2].set_ylim(1e-2,1e0)
	# axSL[2].set_xlim(-0.02,0.1)
	axSL[2].set_xlabel(r'$\tau/\beta$')
	axSL[2].set_ylabel(r'$| F_d(\tau) |$')
	#axSL[2].set_aspect('equal', adjustable='box')
	#axSL[2].axis('square')
	axSL[2].legend()
	axSL[2].set_yscale('log')
	
	# axSL[3].semilogy(tau[startT:stopT]/beta, np.abs(np.real(FODtau[startT:stopT])),'p',c=col,label = lab)
	# #axSL[3].semilogy(tau[startT:stopT], conf_fit_GD[startT:stopT],'k--',label = 'ES power law')
	# #axSL[3].semilogy(tau[startT:], -np.imag(Gconf[startT:]),'m.',label = 'ES solution')
	# #axSL[3].semilogy(tau[startT:], alt_conf_fit_G[startT:],'g--', label = 'alt power law')
	# #axSL[3].set_xlim(tau[startT]/2,tau[startT+15])
	# #axSL[3].semilogy(tau[startT:stopT]/beta, np.exp(mT*tau[startT:stopT] + cT), c=col,label=f'Fit with slope {mT:.03f}')
	# #axSL[3].set_ylim(1e-1,1e1)
	# axSL[3].set_xlabel(r'$\tau/\beta$')
	# axSL[3].set_ylabel(r'$-\Re F_{od}(\tau)$')
	# #axSL[3].set_aspect('equal', adjustable='box')
	# #axSL[3].axis('square')
	# axSL[3].legend()
	# axSL[3].set_yscale('log')
