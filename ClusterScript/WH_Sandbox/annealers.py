...
def anneal_J(J_list,GFtaus,Nbig,g,r,mu,beta,lamb,kappa,DUMP=False,path_to_dump=None,savelist=None,calcfe=False,verbose=True):
	if verbose: 
		print("############ Started : target J = , ", J_list[-1], " #############")
	GDtau,GODtau,DDtau,DODtau = GFtaus
	...
	for J in J_list:
		...
		while(diff>err and itern < ITERMAX):
			...
			diffold = diff
			if itern == ITERMAX - 1: 
				print(f"WARNING : CONVERGENCE NOT REACHED FOR BETA = {beta}, J = {J} in J ANNEAL")
			itern+=1
			iterni += 1 
			...
			diff = 0.5*(diffGD+diffDD) # less stringent error metric - faster code
			# diffGD = np.sqrt(np.sum(np.abs(SigmaDtau - 1.0*kappa*(g**2)*DDtau*GDtau)**2))
			# diffDOD = np.sqrt(np.sum(np.abs(PiODtau - 2.0 * g**2 * GODtau * GODtau[::-1])**2))
			# diff = 0.5*(diffGD+diffDOD)
			# if (diff>err or np.isclose(x,1.)==False) and itern < ITERMAX:
...
def YSYKWH_step(beta,GFtaus,Nbig,g,r,mu,lamb,J=0,kappa=1,x=0.01,err=None,DUMP=False,path_to_dump=None,verbose=True,ITERMAX=ITERMAX):
	GDtau,GODtau,DDtau,DODtau = GFtaus #starting guess

	if err == None: err = 1e-2 * (x**2)
	...
	while(diff>err and itern < ITERMAX): 
		...
		# detD = (nu**2 + r - PiDomega)**2 - (J - PiODomega)**2
		GDomega = x*((1j*omega + mu - SigmaDomega)/(detG)) + (1-x)*oldGDomega
		GODomega = x*((lamb + SigmaODomega)/(detG)) + (1-x)*oldGODomega
		# GODomega = x*(-1.*(lamb- SigmaODomega)/(detG)) + (1-x)*oldGODomega
		DDomega = x*((nu**2 + r - PiDomega)/(detD)) + (1-x)*oldDDomega
		# DODomega = x*(-1.*(J- PiODomega)/(detD)) + (1-x)*oldDODomega
		DODomega = x*((J + PiODomega)/(detD)) + (1-x)*oldDODomega
		...
		diff = 0.5*(diffGD+diffDOD) # less stringent error metric - faster code
		if verbose == True :
			print(f'itern  = {itern}, beta = {beta}, g = {g}, diff = {diff}')
	...
	return GDtau,GODtau,DDtau,DODtau
