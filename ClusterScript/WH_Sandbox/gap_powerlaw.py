...
lambsavelist = np.linspace(0.05,0.1,100)
...
m,c = np.polyfit(np.log(np.abs(lamblooplist)), np.log(gaplist),1)
...
ax.loglog(lamblooplist, np.exp(c)*np.abs(lamblooplist)**m, label=f'Fit with slope {m:.03f}')
ax.legend()
plt.show()
